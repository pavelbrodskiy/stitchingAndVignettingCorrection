...
function [ SSR ] = optimization_overlaps_2x2( SA )
    ...
    SSR = sum(sum((E11 * SA(1) - E21 * SA(2) - SA(5) + SA(6)).^2));
    SSR = SSR + sum(sum((E12 * SA(2) - E22 * SA(4) - SA(6) + SA(8)).^2));
    SSR = SSR + sum(sum((E13 * SA(4) - E23 * SA(3) - SA(8) + SA(7)).^2));
    SSR = SSR + sum(sum((E14 * SA(3) - E24 * SA(1) - SA(7) + SA(5)).^2));
